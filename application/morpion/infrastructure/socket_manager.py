...
def setup_morpion_sockets(socketio):
    ...
    def on_join_room_morpion(data):
        ...
        if len(room['players']) < 2 and user_id not in room['players']:
            ...
            rooms[room_id].append(
                {'user_id': user_id, 'sid': request.sid, 'symbol': symbol})
            join_room(room_id)
            ...
            socketio.emit('room_joined_morpion',
                          {'room': room_id, 'player': player_data},
                          room=request.sid)
            if len(room['players']) == 2:
                socketio.emit('start_game_morpion', {'room': room_id},
                              room=room_id)
        else:
            socketio.emit('error_morpion',
    ...
    def on_start_game(data):
        room_id = data['room']
        if room_id in rooms and any(
                player['sid'] == request.sid for player in rooms[room_id]):
    ...
    def on_make_move(data):
        ...
        if room in rooms and any(p['sid'] == request.sid for p in rooms[room]):
            ...
            current_player_sid = next((p['sid'] for p in rooms[room] if
                                       p['symbol'] == game_state[
                                           'current_player']), None)
    ...
    def on_send_message(data):
        ...
        if user_id:
            ...
            socketio.emit('new_message_morpion',
    ...
    def update_game_state(room):
        ...
        current_player = next((p for p in rooms[room] if
                               p['symbol'] == game_state['current_player']),
                              None)
        socketio.emit('update_state_morpion',
                      {**game_state, 'current_player_info': current_player},
                      room=room)
        if game_state['winner']:
            socketio.emit('game_over_morpion',
